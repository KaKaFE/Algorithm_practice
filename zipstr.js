...
function solution(str) {
  if (str.length === 1) return str.length;    // str 이 1인경우 답은 1
  let zipStrArr = []; // 압축된 문자열의 길이를 담아놓는 배열
  for (let i = 1; i <= str.length / 2; i++) {
    let regexp = new RegExp(`\\w{${i}}`, "g");  // 문자열을 단위별로 자르는 정규표현식
    let splitStr = str.match(regexp); 
    let splitStrLeng = splitStr.join("").length; // 단위 별로 자른 문자열과 원 문자의 나머지 비교를 위한 변수
    if (str.length !== splitStrLeng) {
      splitStr.push(
        str.slice(
          str.length - `${str.length - splitStrLeng}`,
          str.length
        )
      );
    } //원문자열과 자른 문자의 나머지를 다시 넣는분기문
    let zipStr = "";  //  압축 문자열저장을 위한 변수
    let zipCount = 0; //  겹치는문자열 카운트 변수
    for (let i = 0; i < splitStr.length; i++) { // 문자열 압축 반복문
      if (splitStr[i] === splitStr[i + 1]) zipCount++; // 앞뒤 문자열이 같을때 카운트 변수를 증가
      else {
        if (zipCount !== 0) zipStr += zipCount + 1 + splitStr[i]; // 앞뒤문자열이 같지 않을때 카운트 변수와 문자열을 합해 저장
        else zipStr += splitStr[i]; 
        zipCount = 0; //압축 카운트 초기화
      }
    }
    zipStrArr.push(zipStr.length); // 압축된 문자열의 길이를 정답배열에 push
  }

  zipStrArr.sort((a, b) => a - b); // 정답배열의 경우의 수를 정렬
  return zipStrArr[0]; 
}
